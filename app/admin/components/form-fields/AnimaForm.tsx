...
import { AnimaParams } from '../../types';

interface AnimaFormProps {
  params: AnimaParams;
  onChange: (params: AnimaParams) => void;
}

export default function AnimaForm({ params, onChange }: AnimaFormProps) {
  const update = (key: keyof AnimaParams, value: any) => {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold">Anima 参数</h3>
      ...
      <div><label className="block text-sm font-medium mb-1">NL（可选）</label><textarea rows={2} value={params.nl || ''} onChange={(e) => update('nl', e.target.value)} className="w-full border rounded p-2 dark:bg-card" /></div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div><label className="block text-sm font-medium mb-1">CFG</label><input type="number" step="0.1" value={params.cfg} onChange={(e) => update('cfg', Number(e.target.value))} className="w-full border rounded p-2 dark:bg-card" /></div>
        ...
        <div className="col-span-2"><label className="block text-sm font-medium mb-1">工作流 JSON（可选）</label><textarea rows={6} value={params.workflow_json || ''} onChange={(e) => update('workflow_json', e.target.value)} className="w-full border rounded p-2 font-mono text-sm dark:bg-card" placeholder='{"id": "...", ...}' /></div>
      </div>
    </div>
  );
}